...
public class AltCaps extends ConsoleProgram {
	...
	private String altCaps(String input) {
		...
		/**
		 * Convert the string to lowercase and store it in an array
		 * Determine whether it is a character by ASCll
		 * Traverse the array and change the even letters to uppercase
		 */
		char[] CharArry = input.toLowerCase().toCharArray();  //
		for(int i=0; i<CharArry.length; i++) {
			int Ascll = CharArry[i];
			if(Ascll>=97 && Ascll<=122) {
				num++;
				if(num%2 == 0) {
					Ascll-=32;
					CharArry[i] = (char) Ascll;
				}
				TempStr = CharArry[i]+"";
			} else {
				TempStr = CharArry[i]+"";
